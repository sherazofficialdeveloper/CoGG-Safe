...
export async function sendEmergencySmsToNumbers({phoneNumbers, message, sosId, serviceKey = 'sms', preferredSubscriptionId = null}) {
  ...
  const recipientResults = [...previousRecipients];
  for (const result of newResults) {
    const index = recipientResults.findIndex(
      item => (item.normalizedRecipient || item.phoneNumber) === result.normalizedRecipient,
    );
    if (index >= 0) recipientResults[index] = result;
    else recipientResults.push(result);
  }
