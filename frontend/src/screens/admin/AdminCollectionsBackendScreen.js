  ...
  const saveCollection = async () => {
    ...
    try {
      ...
      setCollections(items => {
        const nextItems = items.map(item => item._id === response.collection._id ? response.collection : item);
        collectionSnapshots.set(token, nextItems);
        return nextItems;
      });
