...
test('emergency call starts only after the audio recording has finished, without waiting for other services', async () => {
  const order = [];
  const result = await activateSosFlow({
    userId: 'user-1',
    collectionId: 'collection-1',
    serviceRunners: {
      backend: async () => ({status: 'COMPLETED', backendId: 'backend-audio-call'}),
      sms: async () => { order.push('sms'); return {status: 'COMPLETED', sentCount: 1}; },
      camera: async () => ({status: 'COMPLETED', frontImagePath: '/tmp/front.jpg', backImagePath: '/tmp/back.jpg'}),
      audio: async () => {
        order.push('audio:start');
        await new Promise(resolve => setTimeout(resolve, 50));
        order.push('audio:end');
        return {status: 'COMPLETED', localPath: '/tmp/audio.m4a'};
      },
      call: async () => { order.push('call'); return {status: 'INITIATED'}; },
      location: async () => ({status: 'FAILED', error: 'No location provider available'}),
    },
  });

  expect(order.indexOf('call')).toBeGreaterThan(order.indexOf('audio:end'));
  expect(order.indexOf('sms')).toBeLessThan(order.indexOf('audio:end'));
  expect(result.event.services.call.status).toBe('INITIATED');
});

test('emergency call still starts when the audio recording fails', async () => {
  const result = await activateSosFlow({
    userId: 'user-1',
    collectionId: 'collection-1',
    serviceRunners: {
      backend: async () => ({status: 'COMPLETED', backendId: 'backend-audio-fail'}),
      audio: async () => { throw new Error('mic busy'); },
      call: async () => ({status: 'INITIATED'}),
    },
  });

  expect(result.event.services.audio.status).toBe('FAILED');
  expect(result.event.services.call.status).toBe('INITIATED');
});
