...
describe('Media upload (front/back image, audio)', () => {
  ...
  test('a user outside the SOS collection cannot retrieve another user\'s uploaded media', async () => {
    const collection = await createCollection();
    const otherCollection = await createCollection();
    const { token: ownerToken } = await createUserAndLogin({ collectionId: collection._id });
    const { token: strangerToken } = await createUserAndLogin({ collectionId: otherCollection._id });
  ...
  test('an active member of the SOS collection can retrieve its audio media', async () => {
    const collection = await createCollection();
    const { token: ownerToken } = await createUserAndLogin({ collectionId: collection._id });
