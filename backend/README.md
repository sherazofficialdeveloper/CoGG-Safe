...
Node.js, Express.js, MongoDB/Mongoose, JavaScript (no TypeScript), JWT auth, bcryptjs, multer (media uploads), nodemailer (SMTP), firebase-admin (FCM push), built-in `fetch` for Twilio REST (SMS/Voice) — no Twilio SDK dependency needed.
...
See `.env.example` for the full list. `MONGODB_URI` and `JWT_SECRET` are required — the app refuses to boot without them (outside of `NODE_ENV=test`). All provider credentials (Twilio, SMTP, Firebase) are optional at boot: an unconfigured provider remains unavailable and is recorded as `unsupported`; no delivery is reported without a real provider response.
...
- **All real provider integrations (Twilio SMS/Voice, SMTP, FCM) are written against each service's documented API but have not been exercised against live accounts** — this build environment has no network access. Test with real credentials before production use.
- **Emergency call cannot confirm the call was answered** — only that Twilio accepted the initiation request (`components.call` = "success" means "initiated", not "completed"). Confirming completion requires a public webhook endpoint, out of scope.
