    ...
    try {
      ...
      if (result?.sos) {
        ...
        const frontUrl = frontComp && frontComp.storageRef
          ? `${buildMediaUrl(API_BASE_URL, sosId, 'frontImage')}?token=${encodeURIComponent(token)}`
          : null;
        const backUrl = backComp && backComp.storageRef
          ? `${buildMediaUrl(API_BASE_URL, sosId, 'backImage')}?token=${encodeURIComponent(token)}`
          : null;

        // For notification recipients, prefer the SOS's token-gated emergency
        // media URL returned by the backend. It is authorized by the SOS
        // emergency token itself, so the user-side player does not depend on
        // react-native-fs attaching a Bearer header to a private media stream.
        // The authenticated route remains the fallback for older SOS records.
